...
def load_basis(n_pix: int, basis_dir: str | None = None) -> tf.Tensor:
    """Retrieve the 1d to 2d basis from the basis_dir.

    If the basis is not found, it is created and saved to the basis_dir.

    Args:
        n_pix: number of pixels per axis
        basis_dir: path to the directory where the basis is saved.
    ...
    basis_file_path = os.path.join(basis_dir, f"r2c_basis_{n_pix // 2}")
...
def radius_to_circle_basis(radius_size: int) -> tf.Tensor:
    """Create a basis to map a 1d field to a 2d field.

    Args:
        radius_size: number of pixels in the radius

    Returns:
        tf.Tensor: the 1d to 2d basis
    """
    x = tf.linspace(-1.0, 1.0, radius_size * 2)
    y = tf.linspace(-1.0, 1.0, radius_size * 2)
