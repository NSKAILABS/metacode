"""Fabrication-focused MCP server — phase mask + GDS export tools."""
...
TOOLS = ["generate_phase_mask", "export_gds", "list_artifacts"]
...
def main() -> None:
    ...
    p.add_argument("--port", type=int, default=settings.server.port + 4)
    args = p.parse_args()

    controller = MetaOpticsController.build()
    mcp = FastMCP("MetaOpticsAI-Fabrication")
