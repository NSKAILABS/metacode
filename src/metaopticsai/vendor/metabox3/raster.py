"""
Rasterization functions for metabox.

This module provides classes and functions for rasterizing shapes onto a canvas.
...
import sys 
if TYPE_CHECKING:
    from . import CoordType, ParameterType
...
class Polygon(Shape):
    ...
    Args:
        value: The value/color of the polygon.
        vertices: List of (x, y) coordinate tuples defining the polygon vertices.
                  (Alias: 'points' is also accepted)
    """
    vertices: list["CoordType"] | None = None
    points: list["CoordType"] | None = None  # Alias for vertices

    def __post_init__(self):
        Shape.__post_init__(self)
        # Handle both 'vertices' and 'points' as parameter names
        if self.vertices is None and self.points is None:
            raise ValueError("Either 'vertices' or 'points' must be provided.")
        ...
        self.vertices = [_floatt(p) for p in self.vertices]
        self.points = self.vertices  # Keep both in sync
...
class Circle(Shape):
    ...
    Attributes:
        value: The value/color of the circle.
...
class Rectangle(Shape):
    ...
    Attributes:
        value: The value/color of the rectangle.
...
class Canvas:
    ...
    def rasterize(self, shape_list: list[Shape]) -> tf.Tensor:
        ...
        Returns:
            The rasterized canvas as a tensor.
        """
    ...
    def merge_shape(self, shape: Shape, enforce_4fold_symmetry: bool) -> tf.Tensor:
        ...
        Args:
            ...
            enforce_4fold_symmetry: Whether to enforce 4-fold symmetry.
        """
    ...
    def add_point(self, p: tuple[float, float], radius: float = 0.5) -> None:
    ...
    def add_regular_polygon(
        ...
        """
        self.map = _add_regular_polygon(self, center, radius, n).map
    ...
    def add_circle(
        ...
        Args:
            ...
            radius: The radius of the circle.
        """
        self.map = _add_circle(self, center, radius).map


def _is_complex(x: "ParameterType") -> bool:
...
def _floatt(x: "CoordType") -> tuple[tf.Tensor, tf.Tensor]:
    ...
    Returns:
        The cleaned up coordinates as float32 tensors.
    """
    a, b = tf.math.real(x[0]), tf.math.real(x[1])
    return (tf.cast(a, tf.float32), tf.cast(b, tf.float32))


def _blank_canvas_like(canvas: Canvas) -> Canvas:
...
def _add_point(canvas: Canvas, p: "CoordType", radius: "ParameterType" = 0.5) -> Canvas:
    """Adds a point to the canvas.
...
def _add_circle(canvas: Canvas, center: "CoordType", radius: "ParameterType") -> Canvas:
    """Adds a circle to the canvas.
...
def _is_convex(canvas: Canvas, points: list[tuple[float, float]]) -> bool:
...
def _add_convex_polygon(canvas: Canvas, points: list["CoordType"]) -> Canvas:
...
def rectangle_to_vertices(
    ...
    """Converts rectangle parameters to vertices.
...
def _add_regular_star(
    ...
    Args:
        ...
        keep_positive: Whether to keep positive values.
        apply_threshold: Whether to apply threshold.
