"""
RCWA TensorFlow Implementation.

Original work by Shane Colburn (Email: scolbur2@uw.edu)
...
Updated for Python 3.11.9+ compatibility.
"""
...
def expand_and_tile_np(
    ...
    Args:
        array: A numpy array of shape (Nx, Ny).
        batchSize: The batch size.
        pixelsX: Number of pixels in X direction.
        pixelsY: Number of pixels in Y direction.
        
    Returns:
        A numpy array of shape (batchSize, pixelsX, pixelsY, Nx, Ny) with
...
@tf.custom_gradient
def eig_general(A: tf.Tensor, eps: float = 1e-6):
    """
    Computes the eigendecomposition of a batch of matrices.
    
    Same as tf.linalg.eig() but assumes the input shape also has extra dimensions 
    for pixels and layers. This function also provides the reverse mode gradient 
    of the eigendecomposition as derived in 10.1109/ICASSP.2017.7952140.
    
    This applies for general, complex matrices that do not have to be self adjoint.
    This result gives the exact reverse mode gradient for nondegenerate eigenvalue
    problems. To extend to the case of degenerate eigenvalues common in RCWA, we
    approximate the gradient by a Lorentzian broadening technique that introduces 
    a small error but stabilizes the calculation. Based on 10.1103/PhysRevX.9.031041.
    
    Args:
        A: A tf.Tensor of shape (batchSize, pixelsX, pixelsY, n_layers, Nx, Ny) 
           and dtype tf.complex64 where the last two dimensions define matrices 
           for which we will calculate the eigendecomposition.
        eps: A float defining a regularization parameter used in the denominator 
             of the Lorentzian broadening calculation.

    Returns:
        A tuple containing the eigenvalues and eigenvectors, along with 
        a gradient function.
    """
...
def convmat(A: tf.Tensor, P: int, Q: int) -> tf.Tensor:
    """
    Computes a convolution matrix for a real space matrix A.
    
    A represents either a relative permittivity or permeability distribution 
    for a set of pixels, layers, and batch.
    
    Args:
        A: A tf.Tensor of dtype complex. Can be:
           - 2D shape (Nx, Ny) for simple single-layer case
           - 6D shape (batchSize, pixelsX, pixelsY, n_layers, Nx, Ny) for full batch
    ...
    Returns:
        For 2D input: tf.Tensor of shape (P * Q, P * Q)
        For 6D input: tf.Tensor of shape (batchSize, pixelsX, pixelsY, n_layers, P * Q, P * Q)
    """
    # Handle 2D input case (simple single layer)
    original_ndim = len(A.shape)
    if original_ndim == 2:
        Nx, Ny = A.shape
        # Expand to 6D: (1, 1, 1, 1, Nx, Ny)
        A = A[tf.newaxis, tf.newaxis, tf.newaxis, tf.newaxis, :, :]
        batchSize, pixelsX, pixelsY, n_layers = 1, 1, 1, 1
    elif original_ndim == 6:
        batchSize, pixelsX, pixelsY, n_layers, Nx, Ny = A.shape
    else:
        raise ValueError(f"Input A must be 2D or 6D, got {original_ndim}D")

    # Compute indices of spatial harmonics
    NH = P * Q  # total number of harmonics
    p_max = np.floor(P / 2.0)
    q_max = np.floor(Q / 2.0)  # Fixed: was P, should be Q
    ...
    convMatrixShape = (batchSize, pixelsX, pixelsY, n_layers, P * Q, P * Q)
    matrixStack = tf.reshape(C, shape=convMatrixShape)

    # For 2D input, return 2D output
    if original_ndim == 2:
        matrixStack = matrixStack[0, 0, 0, 0, :, :]
...
def redheffer_star_product(SA: dict, SB: dict) -> dict:
    """
    Computes the Redheffer star product of two block matrices.
    
    This is the result of combining the S-parameters of two systems.
    ...
    Args:
        ...
        SB: A dict of tf.Tensor values specifying the block matrix
            corresponding to the S-parameters of a second system.
            
    Returns:
        A dict of tf.Tensor values specifying the block matrix
        corresponding to the S-parameters of the combined system.
    """
...
def simulate_rcwa(
    ...
    Args:
        incidence_dict: A dict of tf.Tensor values specifying the
            incidence parameters with keys ('wavelength', 'theta', 'phi', 'ptm', 'pte').
        PQ: A tuple of int values specifying the number of spatial
            harmonics in the x and y directions.
        n_cells: An int specifying the number of unit cells.
        n_layers: An int specifying the number of layers in the unit cell.
        layer_thicknesses: A tf.Tensor specifying the thickness of each layer.
        L_xy: A tuple specifying the size of the unit cell in x and y directions.
        er1: The relative permittivity of the reflection region.
        er2: The relative permittivity of the transmission region.
        ER_t: A tf.Tensor specifying the relative permittivity of each layer.
        UR_t: A tf.Tensor specifying the relative permeability of each layer.
        refl_n: The refractive index of the reflection region.
        
    Returns:
        A dict containing the diffraction coefficients of the unit cell.
    """
    ...
    # Scattering matrices for the layers
